...
export async function runAgent({
  ...
  console.log(`🧠 [FIZZBUZZ] Starting validation for number ${message.number}`);
  ...
  while (true) {
    ...
    if (response.content) {
      ...
      console.log(`✨ [FIZZBUZZ] Validation completed`);
      return getMessages();
    }
    ...
    if (response.tool_calls) {
      ...
      console.log(`🔧 [FIZZBUZZ] Executing tool: ${toolName}`);
      ...
      if (toolImplementations[toolName]) {
        ...
        console.error(`❌ [FIZZBUZZ] Unknown tool: ${toolName}`);
      }
    }
  }
}
