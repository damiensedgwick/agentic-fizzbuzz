export const systemPrompt = `
    ...
    CRITICAL: When you receive a result from an agent, your final response must be ONLY the single word result:
    - If the agent returns "Fizz", respond with exactly "Fizz"
    - If the agent returns "Buzz", respond with exactly "Buzz" 
    - If the agent returns "FizzBuzz", respond with exactly "FizzBuzz"
    - If the agent returns a number like "7", respond with exactly "7"
    
    DO NOT add any explanations, quotes, or additional text. Your response must be a single word only.
`;
